...
def encrypter(
    ...
    def key_generator(passes_list: list, current_seed: int, salt_positions: list = None) -> tuple:
    ...
    if not no_salt:
        ...
        key_result = key_generator(salt_result[1], seed, salt_result[2])
    else:
        ciphertext = "".join(crude_ciphertext_list)
        key_result = key_generator(pass_, seed)

    # Saída final
    if debug_mode:
        return (
            f"\nPlaintext: {cor['blu'] + plaintext + cor['pad']}\n\n"
            f"Ciphertext: {', '.join(p for p in crude_ciphertext_list) if no_salt else ', '.join(p for p in salt_result[0])}\n\n"
