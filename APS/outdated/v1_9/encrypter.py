...
def encrypter(
    ...
    def create_salt(ciphertext_list: list, current_pass: list, current_seed: int) -> tuple:
        ...
        for _ in range(salt_leng):
            ...
            tb = gerar_tabelas(current_seed, [salt_pass])
            random_char = chr(random.randint(65, 90))
            
            if debug_mode:
                salt_passes.insert(posicao, cor["red"] + str(salt_pass).zfill(3) + cor["pad"])
                salt_ciphertext_list.insert(posicao, cor["red"] + tb[0][salt_pass][random_char] + cor["pad"])
                posicoes.append(cor["cya"] + str(len(str(posicao))).zfill(3) + cor["pad"])
                ...
                posicoes.append(str(len(posicao)))
                posicoes.append(posicao)

        return salt_ciphertext_list, salt_passes, posicoes

    def key_generator(passes_list: list, current_seed: int, salt_positions: list = None, padding:str = '') -> tuple:
        ...
        poli_salt = [str(s).zfill(10) for s in salt_positions] if salt_positions else []
        ...
        if debug_mode:
            ...
            poli_salt = [cor["yel"] + p + cor["pad"] for p in poli_salt]

        # Gera chave detalhada (apenas para debug)
        crude_key = (
            f"\nsalt: lol_salt: {', '.join(lol_salt)}, salt_l: {', '.join(salt_l)}, \n"
            f"posições salt: {', '.join(poli_salt)}\n\n"
            f"passes: lolp: {lolp}, pl: {pl}, \npasses: {', '.join(poli_passes)}\n\n"
    ...
    if not no_salt:
        ...
        if (len(ciphertext) % 20) == 0:
            key_result = key_generator(passes_list=salt_result[1], current_seed=seed, salt_positions=salt_result[2])
        else:
            padding = (((len(ciphertext) % 20) - 20) * -1)
            ciphertext += padding*"1"
            key_result = key_generator(passes_list=salt_result[1], current_seed=seed, salt_positions=salt_result[2], padding=str(padding))
    else:
        ciphertext = "".join(crude_ciphertext_list)
        if (len(ciphertext) % 20) == 0:
            key_result = key_generator(passes_list=salt_result[1], current_seed=seed)
        else:
            padding = ((len(ciphertext) % 20) - 20) * -1
            ciphertext += padding*"1"
            key_result = key_generator(passes_list=pass_, current_seed=seed, padding=str(padding))
...
if __name__ == "__main__":
    encry = encrypter(plaintext="axabaci", max_table_leng=200, debug_mode=True, no_salt=False)
    print(encry)